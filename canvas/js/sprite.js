function Sprite(numberId, id, data, repeat, stopped) {
    thisInstance = this;
    var isStopped = stopped;
    var redirect = undefined;
    var repeatFromFrame = 0;

    var st = st || {};
    st.hash = {};
    st.frameCount = data.FrameCount;
    st.defineShapes = {};
    st.defineSprites = {};
    st.init = function () {
        st.playList = data;
    ...
    st.processFont = function (a) {
        ...
        for (var c = a.fonts.length; d < c; d++) {
            var g = a.fonts[d], e = st.createTag("glyph");
    ...
    st.processText = function (a) {
        for (var b = st.createTag("g"), d = st.createTag("defs"), c = st.createTag("g"), g = 0; a.records[g]; g++) {
            var e = a.records[g];
            if (a.texttype == "html") {
                var h = a.bound, m = document.createTextNode(e.text), f = document.createElement("span");
    ...
    st.processSprite = function (a) {
        var b = st.createTag("g"), d = st.createTag("defs"), c = st.createTag("g");
    ...
    st.processShape = function () {
        var a = function (a, b, g) {
            ...
            for (var h = a.length; e < h; e++) {
                var m = a[e], f = st.createTag("stop"), j;
        ...
        return function (d) {
            var c = st.createTag("g"), g = st.createTag("defs"), e = st.createTag("g");
            c.appendChild(g);
            c.appendChild(e);
            g = d.paths;
            for (var h = 0, m = g.length; h < m; h++) {
                var f = g[h], j = st.createTag("path");
                ...
                if (typeof f.fill !== "undefined") {
                    ...
                    if (i) {
                        var o = j, l = k;
                        k = i.type;
                        if (k == 1)o.setAttribute("fill", i.color), o.setAttribute("fill-opacity", typeof i["fill-opacity"] == "undefined" ? 1 : i["fill-opacity"]); else {
                            n = "f" + l + ":" + n;
                            l = "";
                            switch (k) {
                                case 2:
    ...
    st.filterElement = function (a, b) {
        var d = {"0": "Shadow", "1": "Blur", "2": "Glow", "3": "Bevel"}, c = function (a) {
        ...
        this.Rgb = function (a) {
            var b = ["R", "G", "B"], c = st.createTag("feComponentTransfer");
            c.setAttribute("in", a);
        ...
        this.init = function () {
            var a = this.filters.rect, b = this.filters.record;
    ...
    st.sprite.prototype.showFrame = function () {
        ...
        if (isStopped == false || this.frame == 0) {

            /*if(this.frame == 0 && repeatFromFrame > 0){
             this.frame = repeatFromFrame
             }*/
//console.log(this.frame)
//console.log(this.frame)
            ...
            //console.log(a)
            a = a[a.length - 1];
            if ("action"in a)for (var b = 0; b < a.action.length; b++)this.action(a.action[b]);
            this._showFrame();
            this.pause || this.frame++;
            //console.log(this.frame+" == "+this.frames.length)
            if (this.frame == this.frames.length) {
                //if (repeat === false && this.frames.length != 1){
                if (repeat === false && this.frames.length == st.frameCount){
...
//console.log(repeatFromFrame)
    ...
    st.sprite.prototype.replaceShape = function (a) {
        var b = this.par.firstChild.nextSibling, d = this.createShape(a);
        if (this.depths[a.depth]) {
            var c = this.par.getAttribute("id") + ":" + a.depth + ":" + this.depths[a.depth], g = document.getElementById(c);
            if (c == d.id)return g;
            //console.log(c)
            //console.log(g)
            b.replaceChild(d, g);
    ...
    st.sprite.prototype.createShape = function (a) {
        var b = this.getNode(a), d = this.par.getAttribute("id") + ":" + a.depth + ":" + a.id;
    ...
    st.sprite.prototype.removeShape = function (a) {
        if (this.depths[a]) {
            var b = this.depths[a], d = this.par.firstChild.nextSibling;
    ...
    st.sprite.prototype.moveShape = function (a, b) {
        if (a.nodeName.toLowerCase() == "g")a.firstChild.nextSibling.setAttribute("transform", "matrix(" + b + ")"); else for (var d = a.childNodes, c = 0, g = d.length; c < g; c++)d[c].setAttribute("transform", "matrix(" + b + ")")
    };
    ...
    st.sprite.prototype.colorTransform = function () {
        function a(a, d) {
            var c = a.match(/rgb\(([^(]+)\)/)[1].split(","), g = Math.max(0, Math.min((c[0] - 0) * d.RedMultTerm / 256 + d.RedAddTerm, 255)), e = Math.max(0, Math.min((c[1] - 0) * d.GreenMultTerm / 256 + d.GreenAddTerm, 255));
            ...
            if ((c == "shape" || c == "text") && b.nodeName.toLowerCase() != "clippath") {
                ...
                try {
                    for (var g = 0, e = c.childNodes.length; g < e; g++) {
                        var h = c.childNodes[g], m = d, f = h.hasAttribute("stroke") ? "stroke" : "fill", j = h.getAttribute(f);
                        if (j)switch (j.match(/^\S{3}/)[0]) {
                            ...
                            case "url":
                                ...
                                if (r.nodeName != "pattern") {
                                    var v =
                                        i.parentNode.previousSibling, p = i.parentNode.parentNode.getAttribute("id") + o;
                                    if (document.getElementById(p))var q = document.getElementById(p); else q = r.cloneNode(!0), q.setAttribute("id", p), v.appendChild(q);
                                    var u = r.childNodes, w = q.childNodes;
                                    l = 0;
                                    for (var x = u.length; l < x; l++) {
                                        var y = u[l].getAttribute("stop-color"), s = a(y, m);
    ...
    this.redirect = function (r) {
        redirect = r;

    };
    this.repeatFromFrame = function (r) {
        repeatFromFrame = r;
    ...
    st.setBackground = function (a) {
        var b = st.createTag("rect"), d = {x: data.FrameRect.xmin, y: data.FrameRect.ymin, width: data.FrameWidth, height: data.FrameHeight, fill: data.BackgroundColor, "fill-opacity": 0}, c;
